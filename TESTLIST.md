...
1. Исходный код: репозиторий `src/*.ts`, `*.spec.ts`.  
2. Покрытие: `npm test` — внизу таблица по `subscription-price-engine.ts`.  
3. Mountebank (если требуется отдельно): `mb` → `npm run mb:post-imposter` → `npm run demo:currency-mock`.
